...
!   2019-xx-xx  CAPS(T. Supinie)  -Add variables and modules for dbz operator using radaremul libary
...
!$$$
  ...
  use directDA_radaruse_mod, only: l_use_cvpqx, l_use_cvpqx_pval, l_gpht2gmht, lvldbg, l_set_oerr_ratio_dbz
...
! modules added for dbz operator using radaremul libary
  use radaremul_cst
  use dualpolepara, only: t_obs_dual, t_para_dsd, init_refl, init_para_dsd, calcMDR, &
...
! used for multi-moments schemes in radaremul library
...
! variables added for dbz operator using radaremul libary
...
!        "bigger" tiny value in read_wrf_mass_guess.F90 for log-transformed qx
           if ( l_use_cvpqx_pval .eq. 0. ) then ! CVlogq
              ...
              qrexp=(l_use_cvpqx_pval*qrges+1)**(1/l_use_cvpqx_pval)
              qsexp=(l_use_cvpqx_pval*qsges+1)**(1/l_use_cvpqx_pval)
              qgexp=(l_use_cvpqx_pval*qgges+1)**(1/l_use_cvpqx_pval)
...
!    array qscalar is used in dbz forward operator in radaremul package    
...
!             rain
              if ( l_use_cvpqx_pval .eq. 0. ) then ! CVlogq
                 ...
                 jqr_num = ten*Cr*((RHO)**Pr)*Pr*((qrexp)**(Pr-l_use_cvpqx_pval))
              end if
...
!                no melting: dry snow at any temperature
                 if ( l_use_cvpqx_pval .eq. 0. ) then ! CVlogq
                    ...
                    jqs_num =ten*Cs_dry*((RHO)**Ps_dry)*Ps_dry*((qsexp)**(Ps_dry-l_use_cvpqx_pval))
                 end if
              else if ( i_melt_snow  .eq. 100 ) then
!                melting: wet snow at any temperature
                 if ( l_use_cvpqx_pval .eq. 0. ) then ! CVlogq
                    ...
                    jqs_num=ten*Cs_wet*((RHO)**Ps_wet)*Ps_wet*((qsexp)**(Ps_wet-l_use_cvpqx_pval))
                 end if
              else
!                melting: depending on temperature
                 if (T1D < 273.15_r_kind) then
...
!                no melting: dry grauple/hail at any temperature
                 if ( l_use_cvpqx_pval .eq. 0. ) then ! CVlogq
                    ...
                    jqg_num=ten*Cg_dry*((RHO)**Pg_dry)*Pg_dry*((qgexp)**(Pg_dry-l_use_cvpqx_pval))
                 end if
              else if ( i_melt_graupel  .eq. 100 ) then
!                melting: wet graupel at any temperature
                 if ( l_use_cvpqx_pval .eq. 0. ) then ! CVlogq
                    ...
                    jqg_num=ten*Cg_wet*((RHO)**Pg_wet)*Pg_wet*((qgexp)**(Pg_wet-l_use_cvpqx_pval))
                 end if
              else
!                melting: depending on the temperature
                 if (T1D < (273.15_r_kind - 2.5_r_kind)) then
                    ...
                    if ( l_use_cvpqx_pval .eq. 0. ) then ! CVlogq
                     ...
                     if ( l_use_cvpqx_pval .eq. 0. ) then ! CVlogq
                        ...
                        jqg_num_dry=ten*Cg_dry*((RHO)**Pg_dry)*Pg_dry*((qgexp)**(Pg_dry-l_use_cvpqx_pval))
                        jqg_num_wet=ten*Cg_wet*((RHO)**Pg_wet)*Pg_wet*((qgexp)**(Pg_wet-l_use_cvpqx_pval))
                     end if
...
!               rain
                if ( qrexp > 10E-7 .and. qscalar(P_NR) > 20 ) then
                   jqr_num = (2*720*RHO**2_r_kind*qrexp**(2_r_kind-l_use_cvpqx_pval)/(pi**2_r_kind*rhor**2_r_kind*qscalar(P_NR)))*ten**12_r_kind*ten**6_r_kind
                   if (l_cvpnr) then
                     jqnr_num = (-720*RHO**2_r_kind*qrexp**2_r_kind/(pi**2_r_kind*rhor**2_r_kind*qscalar(P_NR)**(1_r_kind+cvpnr_pval)))*ten**12_r_kind*ten**6_r_kind
                   else
...
!               snow
                if (qsexp > 10E-7) then
                   jqs_num=ten**(1.602_r_kind*qsexp**0.56_r_kind*1000**0.56_r_kind)*log(ten)*1.602*0.56*1000**0.56_r_kind*qsexp**(0.56_r_kind-l_use_cvpqx_pval)
                else
                   jqs_num = 10E-8
                endif
...
!               graupel/hail
               ...
               if ( qgexp > 10E-7 ) then
                  jqg_num=ten*Cg_wet*((RHO)**1.75_r_kind)*Pg_wet*((qgexp)**(Pg_wet-l_use_cvpqx_pval))
               else
                  jqg_num=10E-8
               endif
...
!              rain
               if ( qrexp > 10E-7 .and. qscalar(P_NR) > 1 ) then
                  jqr_num = (2_r_kind*720*RHO**2_r_kind*qrexp/(pi**2_r_kind*rhor**2_r_kind*qscalar(P_NR)))*ten**12_r_kind*ten**6_r_kind
                  jqnr_num = (-720*RHO**2_r_kind*qrexp**2_r_kind/(pi**2_r_kind*rhor**2_r_kind*qscalar(P_NR)**2_r_kind))*ten**12_r_kind*ten**6_r_kind
               else
                  jqr_num = 10E-8
                  jqnr_num = 10E-8
...
!              snow
               if (qsexp > 10E-7) then
                  jqs_num = ten**(1.602_r_kind*(qsexp*1000)**0.56_r_kind)*log(ten)*1.602*0.56*1000**0.56*qsexp**(-0.44)
               else
                  jqs_num = 10E-8
               endif
...
!            graupel/hail
               if ( qgexp > 10E-7 ) then
                  jqg_num = (ten**12_r_kind*RHO**1.75_r_kind*qgexp**1.5_r_kind*720*2.5/(pi**1.75_r_kind*(200/(0.1*0.2))**0.75_r_kind*rhog**1.9_r_kind))*ten**6_r_kind
               else
                  jqg_num=10E-8
               endif
