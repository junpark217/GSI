...
!     l_decouple_sf_tps- de-couple the correlation/balance between s.f. and temperature, ps
...
!     l_use_cvpqx     - use power transform option to qx (qr/qs/qg)
!     for more information, please refer to directDA_radaruse_mod.f90
!
  namelist/radaruse_directDA/mphyopt,oe_rw,oe_dbz,l_set_be_rw,l_set_be_dbz,   &
                         ...
                         rw_obs4wrd_bmwth, lvldbg,                        &
                         l_correct_azmu, l_correct_tilt, i_correct_tilt,  &
                         l_azm_east1st, l_use_cvpqx, l_use_cvpqx_pval,    &
                         l_plt_be_stats,                                  &
...
! If reflectivity is intended to be assimilated, beta_s0 should be zero.
  if ( beta_s0 > 0.0_r_kind )then
    if ( .not.l_use_rw_columntilt .or. .not.l_use_dbz_directDA) then  ! skipped in case of direct reflectivity DA because it works in Envar and hybrid
       do i=1,ndat
