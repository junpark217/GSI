...
!               write(6,*) 'read_radarref_directDA: single point/column obs run'
                do k=1,numlvl
...
!                        NINT(hgt_ref(k)) .eq. 8000)               ) then
                        ...
                        write(6,*) 'read_radarref_directDA: found the pseudo single(column) dbz obs:',ref3d_column(1:2,ntb),hgt_ref(k)
                    else
