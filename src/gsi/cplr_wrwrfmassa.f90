...
!    The 7 3D cloud analysis fields are: ql,qi,qr,qs,qg,qnr,tt
       if(l_hydrometeor_bkio .and. n_actual_clouds>0) num_mass_fields=num_mass_fields + 7*lm !CCT modified
    else
!    The 9 3D cloud analysis fields are: ql,qi,qr,qs,qg,qnr,qni,qnc,tt
  ...
  ! for hydrometeors      
             if(l_hydrometeor_bkio .and. n_actual_clouds>0) then
! --- direct reflectivity DA---> run CV transform on hydrometeor variables
                if ( l_use_dbz_directDA) then
                   all_loc(j,i,kqc)=ges_qc(j,i,k)
                   all_loc(j,i,kqi)=ges_qi(j,i,k)
                   all_loc(j,i,ktt)=ges_tten(j,i,k,it)

!                set up the tiniy values for qr/qs/qg and convert log-transformed variable back to normal vaiable
                   qr_tmp=zero
                   qs_tmp=zero
                   qg_tmp=zero
...
!                        convert log(qr/qs/qg) back to qr/qs/qg
                         if (mype==0 .AND. i==3 .AND. j==3 .AND. k==3) then
                            write(6,*)'wrwrfmass_netcdf: convert power transformed(qr/qs/qg) back to qr/qs/qg.'
                         end if
