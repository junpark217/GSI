...
!$$$
  ...
  use directDA_radaruse_mod, only: be_sf,hscl_sf, vscl_sf, be_vp,hscl_vp, vscl_vp, &
                               ...
                               l_use_cvpqx, l_use_cvpqx_pval,&
                               l_cvpnr, cvpnr_pval,&       
                               l_plt_be_stats, l_be_T_dep,&
                               l_use_rw_columntilt, l_use_dbz_directDA, lvldbg
            ...
            do j=1,lat2
                ...
                do i=1,lon2
                    do k=1,nsig
                        ...
                        if (n==nrf3_qr) then
                            if( l_use_cvpqx==.true. .and. l_use_cvpqx_pval==0.5_r_kind) then
                                 call calc_corz_cld_qr(tsen, l_use_cvpqx_pval, corz_cld(i,j,k,1)) ! JP add subroutine to calc 'corz_cld
                                 dssv(j,i,k,n)=dsv_cld(i,k,j) *0.05_r_kind*corz_cld(i,j,k,1) * as3d(n)       
                            else if( l_use_cvpqx==.true. .and. l_use_cvpqx_pval==1.0_r_kind) then
                                 call calc_corz_cld_qr(tsen, l_use_cvpqx_pval, corz_cld(i,j,k,1))
                                 dssv(j,i,k,n)=dsv_cld(i,k,j) * corz_cld(i,j,k,1) * as3d(n)
                            else if( l_use_cvpqx==.true. .and. l_use_cvpqx_pval==0.000001_r_kind) then
                                 call calc_corz_cld_qr(tsen, l_use_cvpqx_pval, corz_cld(i,j,k,1))
                                 dssv(j,i,k,n)=dsv_cld(i,k,j) * corz_cld(i,j,k,1) * as3d(n)
                            else if( l_use_cvpqx==.true. .and. l_use_cvpqx_pval==0.4_r_kind) then
                                 call calc_corz_cld_qr(tsen, l_use_cvpqx_pval, corz_cld(i,j,k,1))
                                 dssv(j,i,k,n)=dsv_cld(i,k,j) *0.05_r_kind* corz_cld(i,j,k,1) * as3d(n)     
                            else
                            ...
                            if( l_use_cvpqx==.true. .and. l_use_cvpqx_pval==0.5_r_kind) then
                                call calc_corz_cld_qs(tsen, l_use_cvpqx_pval, corz_cld(i,j,k,2))
                            ...
                            if( l_use_cvpqx==.true. .and. l_use_cvpqx_pval==0.5_r_kind) then
                                call calc_corz_cld_qg(tsen, l_use_cvpqx_pval, corz_cld(i,j,k,3))
