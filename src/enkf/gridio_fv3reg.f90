...
contains
  subroutine readgriddata(nanal1,nanal2,vars3d,vars2d,n3d,n2d,levels,ndim,ntimes,fileprefixes,filesfcprefixes,reducedgrid,vargrid,qsat)
   ...
   if (qnr_ind > 0) then
    ...
    ! add 'prse' into vargrid if two conditions met.
    if ( l_use_enkf_directZDA .and. prse_ind > 0 ) then

      do k=1,nlevs
        do j=1,ny_res
         do i=1,nx_res
           workvar3d(i,j,k)=(workprsi(i,j,k))
         enddo
        enddo
      enddo
      ...
      do k=1,nlevs
        ...
        do j=1,ny_res
         do i=1,nx_res
            nn=nn+1
            vargrid(nn,levels(prse_ind-1)+k,nb,ne)=workvar3d(i,j,k)
         enddo
        enddo
      enddo
      do k = levels(prse_ind-1)+1, levels(prse_ind)
          if (nproc .eq. 0)                                               &
             write(6,*) 'READFVregional : prse ',                           &
                 & k, minval(vargrid(:,k,nb,ne)), maxval(vargrid(:,k,nb,ne))
      enddo
...
subroutine writegriddata(nanal1,nanal2,vars3d,vars2d,n3d,n2d,levels,ndim,vargrid,no_inflate_flag)
    ...
    integer(i_kind) :: ql_ind, qi_ind, qr_ind, qs_ind, qg_ind, qnr_ind, prse_ind
    ...
    if ( l_use_enkf_directZDA ) then
       prse_ind  = getindex(vars3d, 'prse')  ! QNR (3D) for FV3
    end if
    ...
    if (ps_ind > 0) then
      ...
      ! update 'delp' directly from vargrid (analysis increment)
      if ( l_use_enkf_directZDA .and. prse_ind > 0) then
        ...
        do j=1,ny_res
           do i=1,nx_res
              nn=nn+1
              workinc3d(i,j,k)=vargrid(nn,levels(prse_ind-1)+k,nb,ne) 
              workprsi(i,j,k)=workprsi(i,j,k)+workinc3d(i,j,k)   ! add inc
           enddo
        enddo
        enddo
   
          do k = 1, nlevs-1     ! prse to delp
            workvar3d(:,:,k) = 100. * (workprsi(:,:,k) - workprsi(:,:,k+1))
          end do
          workvar3d(:,:,nlevs) = 100. * (workprsi(:,:,nlevs) - eta1_ll(nlevsp1))
